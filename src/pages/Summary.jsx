    ...
    <div className="w-screen h-screen flex flex-col lg:flex-row items-center lg:items-start justify-center bg-[#001229]">
      ...
      <div className="w-full lg:w-1/2 max-w-[600px] h-auto px-4 py-8 lg:p-16 z-10 flex flex-col justify-start items-start gap-6 lg:gap-8 overflow-y-auto">
        ...
        <div className="w-full flex justify-between items-center">
          <h2 className="text-[18px] font-bold text-white">{summary?.subscriptionPlan?.name}</h2>
          {/* <Link
            to="/select-package"
        ...
        <div className="w-full flex flex-col gap-4">
          ...
          <div className="w-full p-4 bg-[#21344C] rounded-xl border border-[#55C9FA] grid gap-4 grid-cols-1 lg:grid-cols-8">
            <img src={MasterCardIcon} alt="MasterCard" className="col-span-1 lg:col-span-1" />
            <span className="col-span-1 lg:col-span-3 text-xl text-white">****-****-****-{summary?.card?.number}</span>
            <div className="col-span-1 lg:col-span-2">
              <span className="block text-sm font-medium text-white">Name On Card</span>
              <span className="block text-sm text-white">{summary?.card?.name}</span>
            </div>
            <div className="col-span-1 lg:col-span-2">
              <span className="block text-sm font-medium text-white">Expires On</span>
              <span className="block text-sm text-white">{summary?.card?.expireOn}</span>
            </div>
          </div>
        </div>
        ...
        <div className="w-full flex justify-end items-center gap-2">
          <span className="text-[16px] font-medium text-white">Total Amount:</span>
          <span className="text-[24px] font-bold text-white">{summary?.subscriptionPlan?.price}</span>
        ...
        <div className="w-full flex justify-end gap-2">
          ...
          >
            <div className="flex items-center"><span className="mr-1">Save</span>{loading &&(
